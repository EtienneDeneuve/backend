...
* /v1/types endpoint
*/
describe('Endpoint /v1/cmdb/types/2/items', function() {
  it('create a new item', function(done) {
    request
    .post('/v1/cmdb/types/2/items')
  ...
  it('create a new type, but properties.property_id not defined => error', function(done) {
    request
    .post('/v1/cmdb/types/2/items')
    .send({name: 'L0014',properties:[{propertyId:3,value:"serialxxxxxx"}]})
    .set('Accept', 'application/json')
    .set('Authorization', 'Bearer ' + global.token)
    .expect(400)
    .expect('Content-Type', /json/)
  ...
  it('create a new type, but properties.property_id is defined to 0 => error', function(done) {
    request
    .post('/v1/cmdb/types/2/items')
    .send({name: 'L0014',properties:[{property_id:0,value:"serialxxxxxx"}]})
    .set('Accept', 'application/json')
    .set('Authorization', 'Bearer ' + global.token)
    .expect(400)
    .expect('Content-Type', /json/)
  ...
  it('create a new type, but properties.value not defined => error', function(done) {
    request
    .post('/v1/cmdb/types/2/items')
    .send({name: 'L0014',properties:[{property_id:3}]})
    .set('Accept', 'application/json')
    .set('Authorization', 'Bearer ' + global.token)
    .expect(400)
    .expect('Content-Type', /json/)
  ...
  it('create a new type, but properties.value is an integer => error', function(done) {
    request
    .post('/v1/cmdb/types/2/items')
    .send({name: 'L0014',properties:[{property_id:3,value:1000}]})
    .set('Accept', 'application/json')
    .set('Authorization', 'Bearer ' + global.token)
    .expect(400)
    .expect('Content-Type', /json/)
